    ...
    getHomeArtists(latitude, longitude): Promise<Array<ArtistHome>> {
        return new Promise((resolve, reject) => {
            this.artistRepository
                ...
                .subscribe((response) => {
                    this.artistsHome = response.map(item => new ArtistHome(item.photo, item.nbVotes, item.totalVotes/item.nbVotes, item.artist.name, item.artist.description));
    ...
    getDetailArtist(artistName): Promise<Array<ArtistDetail>> {
        return new Promise((resolve, reject) => {
            this.artistRepository
                ...
                .subscribe((response) => {
                    this.artistsDetail = response.map(item => new ArtistDetail(item.photo, item.artist.name, item.artist.description, 
                        item.longDescription, item.site, item.phoneNumber, item.email, item.nbVotes, item.totalVotes/item.nbVotes, item.departments));
